...
final class KeyStore {
	private const OPTION_NAME = 'wpccai_client_key_ring';
	...
	public function prepare_rotation( ?int $now = null ): ?KeyPair {
		if ( ! ( new RotationPolicy() )->rotation_due( $this->current()->created_at, $now ) ) {
			return $this->next();
		}
	...
	public function promote( string $key_id ): bool {
		$ring = $this->load();

		if ( empty( $ring['next']['key_id'] ) || ! hash_equals( (string) $ring['next']['key_id'], $key_id ) ) {
			return false;
		}
		...
		return update_option( self::OPTION_NAME, $ring, false );
	}
	...
	private function encryption_key(): string {
		...
		return SecretBox::derive_key( 'wpccai-client:' . $material );
	}
}
